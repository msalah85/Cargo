...
function onAmountChanged(e) {
    ...
    if ((_isvatable == "true")) {
        ...
        $(e).closest('tr').find('input')[2].value = newVAT; // set new value to Vat textbox in this row.
    }
    else {
        $(e).closest('tr').find('input')[2].value = 0;
    }
}
...
var pageManager = function () {
    ...
    var
                ...
                var detailData = (_id) ? jsn3 : jsn,
                    rows = $(detailData).map(function (i, v) {
                        return $('<tr><td data-expenseid="' + v.ExpenseID + '" data-inv-details-id="' + (v.InvoiceDetailsID ? v.InvoiceDetailsID : 0) + '" class="center">' + (i + 1) + '</td><td>' + v.ExpenseName + '</td>\
                             ...
                             <td><input onchange="onAmountChanged(this)" data-IsVatable="'+ v.IsVatable + '" data-expid="' + v.ExpenseID + '" ' + (v.ParentExpenseID ? (' data-parent-expid="' + v.ParentExpenseID + '"') : '') + ' type="number" value="' + numeral(v.Amount ? v.Amount : v.DefaultValue).format('0.0') + '" /></td>\
                             <td><input readonly data-expid="' + v.ExpenseID + '" ' + (v.ParentExpenseID ? (' data-parent-expid="' + v.ParentExpenseID + '"') : '') + ' type="number" value="' + (v.VAT && v.VAT * 1 > 0 ? numeral(v.VAT).format('0.00') : '') + '" /></td>\
        ...
        BindGrid = function () {
            var VATAmountParam = $('#Amount').val() * 1;
