...
var pageManager = function () {
    ...
    var
        ...
        pageEvents = function () {
            ...
            // update total
            $gridTable.delegate('tr input[type="number"]:not(.vat)', 'change', function (e) {
                var el = $(this);

                // read values of this current Row.
                var _isvatable = el.attr('data-isvatable');
                var _value = el.val();
                if ((_isvatable === "true")) {
                    // applay changes on Vat Amount based on _isvatable.
                    //var nextTXT = parseFloat(el.closest('tr').find('input')[0].value);
                    var newVAT = _value * selfInviceAddManager.VAT;

                    el.closest('tr').find('input')[2].value = newVAT.toFixed(2); // set new value to Vat textbox in this row.
                }
                else {
                    el.closest('tr').find('input')[2].value = 0;
                }
            ...
            if (isValid) {
                ...
                var
                    ...
                    namesMaster = ['InvoiceID', 'ClientID', 'AddDate', 'TotalAmount', 'Profit', 'ContainerNo', 'DeclarationNo',
                        'Notes', 'BillOfEntryDate', 'TransporterID', 'CraneDriverID', 'User'],
                    ...
                    $('#DeclarationNo').val(), $('#Notes').val(), commonManger.dateFormat($('#BillOfEntryDate').val()),
                        TransporterID, CraneDriverID, $('user:eq(0)').text()],
        ...
        bindFormControls = function (d) {
            ...
            // expenses
            if (jsn) {
                var _options = $(jsn).map(function (i, v) { return $('<option ' + (v.IsVatable === 'true' ? 'data-isvatable=' + v.IsVatable : '') + ' />').val(v.ExpenseID).text(v.ExpenseName); }).get();
                ...
                var detailData = _id ? jsn3 : jsn,
                    rows = $(detailData).map(function (i, v) {
                        ...
                        return $('<tr><td data-expenseid="' + v.ExpenseID + '" data-inv-details-id="' + (v.InvoiceDetailsID ? v.InvoiceDetailsID : 0) + '" class="center">' + (i + 1) + '</td><td>' + v.ExpenseName + '</td>\
                             ...
                             <td><input '+ ((v.VAT * 1) > 0 || v.IsVatable === 'true' ? 'data-isvatable="true"' : '') + ' data-expid="' + v.ExpenseID + '" type="number" value="' + numeral(v.Amount ? v.Amount : v.DefaultValue).format('0.0') + '" /></td>\
                             <td><input class="vat" data-expid="' + v.ExpenseID + '" ' + (v.ParentExpenseID ? (' data-parent-expid="' + v.ParentExpenseID + '"') : '') + ' type="number" value="' + ((v.VAT * 1) > 0 ? numeral(v.VAT).format('0.0') : 0) + '" /></td>\
            ...
            if (jsn) {
                ...
                var rows = $(jsn).map(function (i, v) {
                    return $('<tr><td data-expenseid="' + v.ExpenseID + '" data-inv-details-id="' + (v.InvoiceDetailsID ? v.InvoiceDetailsID : 0) + '" class="center">' + (i + 1) + '</td><td>' + v.ExpenseName + '</td>\
                             ...
                             <td><input class="vat" type="number" value="' + numeral(v.VATAmount || 0).format('0.0') + '" /></td>\
                             <td><button class="btn btn-minier btn-danger remove" data-rel="tooltip" data-placement="top" data-original-title="Delete" title="Delete"><i class="fa fa-remove icon-only"></i></button></td></tr>');
                }).get(), isExist = false;
        ...
        showPaymentsTotal = function () {
            ...
            $('#listItems tbody tr').each(function (i, item) {
                var cstVal = $(this).find('td:eq(2) input').val() * 1,
                    ...
                    nodeVat = $(this).find('td:eq(4) input').val() * 1;

                _totalCost = _totalCost + ((cstVal && !isNaN(cstVal)) ? parseFloat(cstVal) : 0); // cost
                _total4Cust = _total4Cust + ((custVal && !isNaN(custVal)) ? parseFloat(custVal) : 0); // amount/customer
                _totalVAT = _totalVAT + ((nodeVat && !isNaN(nodeVat)) ? parseFloat(nodeVat) : 0); // vat
            });

            // show total amount and profit.            
            $('#TotalAmount').text(numeral(_total4Cust + _totalVAT).format('0,0.0')); // show invoice total
        ...
        GetVat = function (d) {
            ...
            selfInviceAddManager.VAT = parseFloat((jsn[1] ? jsn[1].Val : 0) || 0);
        },
